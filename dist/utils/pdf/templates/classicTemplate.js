...
async function generateClassicTemplate(invoice, res, isDownload = false) {
    ...
    doc.font("Times-Bold").fontSize(16).text("INVOICE", 30, 65);
    doc
        .font("Times-Roman")
        .fontSize(10)
        .text(`Invoice Number: ${invoice.invoice_number}`, 30, 85)
        .text(`Client: ${invoice.client.name}`, 20, 100)
        .text(`Invoice Date: ${new Date(invoice.start_date).toLocaleDateString("id-ID")}`, 30, 115);
    doc.fontSize(10);
    doc.text(`Due Date: ${new Date(invoice.due_date).toLocaleDateString("id-ID")}`, 30, 130);
    if (invoice.recurrence_type && invoice.recurrence_interval) {
        doc.text(`Recurring Type: ${invoice.recurrence_type}`, 30, 145);
        doc.text(`Interval: Every ${invoice.recurrence_interval} ${invoice.recurrence_type.toLowerCase()}(s)`, 30, 160);
    }
    doc.moveDown();
    doc.moveTo(25, doc.y).lineTo(545, doc.y).stroke();
    const tableData = {
        headers: [
